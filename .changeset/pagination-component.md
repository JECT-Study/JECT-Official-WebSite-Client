...
전체 페이지 수가 표시 개수보다 크면 연속된 숫자 구간과 말줄임을 표시합니다. 컴포넌트가 유지되는 동안 숫자 구간의 안쪽 페이지를 선택하면 구간을 유지하고, 양끝 페이지를 선택하면 표시 구간을 다시 계산합니다. 처음 표시되는 가운데 구간에서 홀수 표시 개수는 현재 페이지가 중앙에, 짝수 표시 개수는 오른쪽 숫자가 하나 더 많게 배치됩니다. 첫 페이지와 마지막 페이지 근처에서는 숫자를 연속으로 표시합니다.

`page`나 `totalPages` 변경으로 현재 페이지가 표시 가능한 범위로 보정되어도 `onPageChange`를 호출하지 않습니다. `totalPages` 또는 `visiblePageCount`가 바뀌거나 컴포넌트가 다시 마운트되면 현재 페이지에 맞춰 숫자 구간을 다시 배치합니다. 전체 페이지 수가 `0`이었다가 다시 늘어나면 이전 숫자 구간을 초기화합니다. 링크 방식에서 URL을 현재 페이지의 기준으로 쓰려면 소비처가 URL에서 페이지 값을 읽어 `page`로 전달합니다.
