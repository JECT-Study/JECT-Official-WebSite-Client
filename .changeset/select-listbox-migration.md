...
- 컨테이너 역할이 `role="listbox"`로 통일되고, 항목 역할이 `role="checkbox"`, `role="radio"`에서 `role="option"`으로 변경됩니다. 체크박스와 라디오는 선택 상태를 나타내는 시각적 요소로만 동작합니다.
- `ref`가 가리키는 요소가 `role`을 가진 항목 래퍼에서 최상위 컨테이너로 변경됩니다. 항목은 데이터 객체가 되어 개별 `ref`를 받지 않습니다.
