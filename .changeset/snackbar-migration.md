...
Snackbar가 Compound 구조에서 `feedback` prop을 사용하는 단일 컴포넌트 구조로 변경되었습니다. `none`, `positive`, `destructive`, `notifying` 피드백 상태를 지원하고, Snackbar 노출 시간을 `duration`으로 조절할 수 있습니다. 스크린리더 낭독은 시각용 Snackbar 스택과 분리된 live region에서 가장 최근 Snackbar만 읽도록 개선되었습니다.
