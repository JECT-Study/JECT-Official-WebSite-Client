...
`@jects/jds/utils`가 노출하는 타이포 표면이 `getLabelClassName`, `getTitleClassName`, `getBodyClassName` 세 함수와 그 옵션 타입으로 좁혀집니다. recipe를 그대로 내보내던 `typography` 객체, 내부 전용 헬퍼, 제거된 `Label` 컴포넌트에서 남아 있던 타입이 사라집니다. JDS 컴포넌트의 prop과 렌더 결과는 그대로입니다.
...
| `LabelTextAlign`, `LabelCursor`                           | 제거 — `Menu.Category`의 `textAlign`, `cursor` prop은 동작 그대로 |
